...
export const StudyDateParticipantController = {
	async getByStudyDate(req: Request, res: Response) {
		try {
			...
			const participants =
				await StudyDateParticipantService.getByStudyDate(Number(study_date_id));
	...
	async join(req: Request, res: Response) {
		try {
			...
			const id = await StudyDateParticipantService.join(
				body.study_date_id,
				body.user_id
			);
	...
	async leave(req: Request, res: Response) {
		try {
			const body = (req as any).validatedBody;
			await StudyDateParticipantService.leave(
				body.study_date_id,
				body.user_id
			);
	...
	async count(req: Request, res: Response) {
		try {
			...
			const total =
				await StudyDateParticipantService.countByStudyDate(Number(study_date_id));
