...
export const MessageModel = {
  ...
  async getByStudyDate(study_date_id: string): Promise<Message[]> {
...
async create(data: Partial<Message>): Promise<string> {
  ...
  const [rows]: any = await pool.query(
    `SELECT id FROM messages WHERE id = LAST_INSERT_ID()`
  );
  ...
  async getBySender(sender_id: string): Promise<Message[]> {
    const [rows] = await pool.query(
      "SELECT * FROM messages WHERE sender_id = ? ORDER BY sent_at ASC",
      [sender_id]
    );
    return rows as Message[];
  },
